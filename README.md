...
Este proyecto implementa una API REST mínima para gestionar tareas en memoria.  
...
`TaskService` mantiene una lista interna de tareas y un contador incremental de IDs.
Esto significa que los datos **no son persistentes**; reiniciar el servidor borra toda la información.

### Flujo
1. Los **routers** reciben solicitudes HTTP.
2. Realizan validaciones básicas (existencia de la tarea, status code, errores).
3. Llaman a los métodos del **service**.
4. Los servicios manipulan los modelos definidos en **models**.
5. Los routers devuelven la respuesta al cliente con los modelos correctos.
...
|--------|-----------|-------------|
| GET    | `/health` | Verifica estado del servicio, uptime y timestamp |
...
- **Documentación automática Swagger**: [http://localhost:3000/docs](http://localhost:3000/docs)
...
- Al usar almacenamiento en memoria, cada reinicio del servidor reinicia las tareas.
- Puedes conectarlo posteriormente a una base de datos real reemplazando la capa de `services`.
